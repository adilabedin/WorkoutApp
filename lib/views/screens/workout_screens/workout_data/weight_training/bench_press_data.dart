...
class BenchPressData extends StatefulWidget {
  const BenchPressData({Key? key, required this.workoutType}) : super(key: key);

  final String workoutType;

  @override
  State<BenchPressData> createState() => _BenchPressDataState();
}

class _BenchPressDataState extends State<BenchPressData> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Obx(() {
        items = workoutController.benchPress.length;
        progressWTList();
        ...
        return PageView.builder(
          ...
          itemBuilder: (context, index) {
            data = workoutController.benchPress[index];
  ...
  progressWTList() {
    for (int i = 0; i < workoutController.benchPress.length; i++) {
      Rdata.add(ProgressWT(
          workoutName: workoutController.benchPress[i].workoutTime,
          weightused: workoutController.benchPress[i].workoutWeight,
